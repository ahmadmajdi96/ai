...
async def evaluate(payload: dict, tenant_id: str = Depends(require_tenant)):
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        ...
        system_msg = """
You are the BOTTLENECK / THROUGHPUT AGENT.
Your job: Evaluate how this decision impacts line capacity,
bottlenecks, throughput risk.
Return valid JSON:
dimension, rating, reasoning, risk_bottleneck.
'rating' 0-100, higher = good throughput / no choke.
If it harms a critical bottleneck line, rating must be low.
"""
        user_msg = {
            "DECISION": decision,
            "RULES": rules,
